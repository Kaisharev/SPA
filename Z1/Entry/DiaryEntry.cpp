...
std::string DiaryEntry::GetStringifiedEntry(){
    return "ID: " + std::to_string(this->id) 
            + " | " 
            + "Prioritet: " + std::to_string(this->priority)
            +" | " 
            +"Datum: " + this->date.GetDateAsString()
            +" | " 
            + "Vrijeme:" + this->time.GetTimeAsString()
            +" | " 
...
std::string DiaryEntry::LoadEntryFromFile() {
    ...
    if (!inputFile.is_open()) {
        throw std::invalid_argument("Traženin fajl ne postoji!");
    }
