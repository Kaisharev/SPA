...
Date Date::ParseDate (const std::string& date_string) {
    ...
    if (!IsValidDate (std::stoi (regex_result[1]), std::stoi (regex_result[2]), std::stoi (regex_result[3])))
        throw std::invalid_argument ("Uneseni datum nije validan");

    return Date (std::stoi (regex_result[1]), std::stoi (regex_result[2]), std::stoi (regex_result[3]));
}
